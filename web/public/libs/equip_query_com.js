function save_equip_price_info(equipid, price, isProtect, isDraw, endTime) {
  var identify = 'identify_' + equipid
  var ck_value = price
  var ck_time = 10 * (1 / (24 * 60 * 60))
  Cookie.write(identify, ck_value, { duration: ck_time })
  if (isProtect) {
    return handleDraw(isProtect == 'true', isDraw == 'true', endTime)
  }
}
function toggleDialog() {
  $('tipsDialog').toggleClass('hidden')
}
...
function init_sub_nav() {
  var NavManager = function (dom) {
    ...
    if (this.item_dom_list[this.item_index]) this.item_dom_list[this.item_index].className = 'on'
...
function search_to_wtb(wtb_type) {
  window.location =
    CgiRootUrl +
    '/wanttobuy.py?act=search_to_wtb&wtb_type=' +
    wtb_type +
    '&args=' +
    JSON.encode(SearchArg)
}
function init_autocomplete(box, options) {
  if (box) new AutoComplete(box, options)
}
...
function init_gift_select_box() {
  var lihe_equip_types = AUTO_SEARCH_CONFIG.lihe_equip_type || []
  var libi_equip_types = AUTO_SEARCH_CONFIG.libi_equip_type || []
  var resultObject = {}
  lihe_equip_types.each(function (item) {
    resultObject[item.value] = item.label
  })
  libi_equip_types.each(function (item) {
    resultObject[item.value] = item.label
  })
  var handle_pet_search = function (keyword) {
    var result = new Array()
    for (var pet_type in resultObject) {
      if (resultObject[pet_type].indexOf(keyword) != -1) {
        var type_name = resultObject[pet_type]
        if (result.indexOf(type_name) == -1) {
          result.push(type_name)
        }
      }
  ...
  var do_pet_search = function (e) {}
  init_autocomplete($('gift_select_box'), {
    startPoint: 1,
    promptNum: 20,
    handle_func: handle_pet_search,
    callback: do_pet_search
...
function init_msyj_select_box() {
  var items = MO_SHOU_YAO_JUE.concat(SaleNeidanSkills).concat(SuperYaoJue)
...
function search_pet(page) {
  ...
  if (growth > 0) {
    if (growth > 1.3) {
      alert('召唤兽成长只能输入0到1.3之间的正数，且小数点后不超过3位。请您重新输入。')
      return
    }
    arg['growth'] = Math.round(growth * 1000)
  }
  if ($('pet_skill_num').value) {
    arg['skill_num'] = $('pet_skill_num').value
    if ($('no_include_sp_skill').checked) {
  ...
  if (arg['price_min'] == 0) {
    delete arg['price_min']
  }
  if (arg['price_max'] == MaxTradeYuan) {
    delete arg['price_max']
  }
  if (arg['price_min']) {
...
function search_lingshi(page) {
  ...
  if (!isNaN(level_min)) {
    arg['equip_level_min'] = level_min
  }
  if (!isNaN(level_max)) {
    arg['equip_level_max'] = level_max
  }
  var txt_int_items = [
    ['price_min', 0, MaxTradeYuan, '价格'],
    ['price_max', 0, MaxTradeYuan, '价格'],
...
function searchLocalData(keyword, liheTypes, libiTypes) {
  var foundValues = []
  liheTypes.each(function (item) {
    if (item.label.toLowerCase().indexOf(keyword.toLowerCase()) !== -1) {
      foundValues.push(item.value)
    }
  })
  libiTypes.each(function (item) {
    if (item.label.toLowerCase().indexOf(keyword.toLowerCase()) !== -1) {
      foundValues.push(item.value)
    }
  })
...
function search_gift_box(page) {
  var arg = { act: 'query', kindid: -4 }
  if (page) {
    arg['page'] = page
  }
  var selected_type = get_radio_value('selected_type')
  arg['selected_type'] = selected_type
  var equip_type = []
  var lihe_equip_type = $('lihe_equip_type').value
  var libi_equip_type = $('libi_equip_type').value
  var gift_select_box = $('gift_select_box').value
  if (selected_type === 'lihe_equip_type') {
    arg['lihe_equip_type'] = lihe_equip_type
    equip_type.push(lihe_equip_type)
  }
  if (selected_type === 'libi_equip_type') {
    arg['libi_equip_type'] = libi_equip_type
    equip_type.push(libi_equip_type)
  }
  if (selected_type === 'lihe_libi_equip_type') {
    var lihe_equip_types = AUTO_SEARCH_CONFIG.lihe_equip_type || []
    var libi_equip_types = AUTO_SEARCH_CONFIG.libi_equip_type || []
    var resultObject = {}
    lihe_equip_types.each(function (item) {
    ...
    arg['gift_select_box'] = encodeURIComponent(gift_select_box)
    equip_type.push(resultObject[gift_select_box])
  }
  if (equip_type.length) {
    arg['equip_type'] = equip_type.join(',')
  }
  submit_advance_search(arg)
}
...
function search_equip(page, act) {
  ...
  if (kind == 'search_role_equip') {
    ...
    if (s_name.length != 0) {
      ...
      if (SuperYaoJue.contains(s_name)) {
        arg['s_type'] = '56870'
        arg['skill_name'] = s_name.toBase64()
      }
    }
    ...
    if (arg['s_type'] == '50344') {
      arg['skill_text'] = $('zhongling_stone_tx').value
      arg['equip_level'] = $('sel_zhongling_stone_level').value
    } else if (arg['s_type'] == '612') {
      arg['skill_text'] = $('sel_guidebook_zhizao').value
    } else if (arg['s_type'] == '18126') {
      arg['skill_text'] = $('sel_guidebook_lingshi').value
    } else if (arg['s_type'] == '6904') {
      arg['skill_text'] = $('sel_lingxiyu_effect').value
    } else if (arg['s_type'] == '56973') {
      arg['skill_text'] = $('sel_xunlingzhu').value
      arg['equip_level'] = $('sel_xunlingzhu_level').value
    } else {
      arg['skill_text'] = $('sel_dianhuashi_skill').value
    }
...
function search_role(page) {
  var arg = { act: 'search_role', school: $('role_school').value, race: $('role_type').value }
  if (page) {
    arg['page'] = page
  }
  var txt_int_items = [
  ...
  var confs = ['expt_gongji', 'expt_fangyu', 'expt_fashu', 'expt_kangfa', 'expt_lieshu']
  ...
  if ($('original_school_skill_0').checked) {
    arg['use_original_school_skill_level'] = '1'
  }
  submit_advance_search(arg)
}
...
function init_advance_search_box() {
  ...
  var handler = function (keyword, callback) {
    ...
    var result = cache[keyword]
    if (result) {
      return result
    }
    ajax && ajax.cancel()
    var proxy = getProxyAjax(CgiRootUrl + '/equipquery.py', {
      act: 'get_hotwords',
      keyword: keyword
    })
    var dispatcher = proxy.dispatcher
    ajax = proxy.ajax
    dispatcher.addEvent('complete', function (data) {
      if (data && data.hotwords) {
        result = cache[keyword] = data.hotwords || []
        callback(result)
      } else {
      }
    })
...
function search_by_equip_name(cur_equip_name) {
  return search_auto_complete(document.search_browse_f)
}
...
function reset_material_info() {
  ...
  $('sel_lingxiyu_effect').setStyle('display', 'none')
}
function set_equip_search_event() {
  $('sel_material_s_type') &&
    $('sel_material_s_type')
      .addEvent('change', function () {
        if (['4202', '50344', '1', '2', '56973'].contains(this.value)) {
          reset_material_info()
          $('sel_material_level').set('value', '')
        } else if (this.value == '4034') {
          reset_material_info()
          $('sel_dianhuashi_skill').setStyle('display', '')
        } else if (this.value == '6904') {
          reset_material_info()
          $('sel_lingxiyu_effect').setStyle('display', '')
          $('sel_material_level').set('value', '')
        } else {
          reset_material_info()
        ...
        if (this.value == 612) {
          $('sel_guidebook_zhizao').setStyle('display', '')
        } else {
          $('sel_guidebook_zhizao').setStyle('display', 'none')
        }
        if (this.value == 18126) {
          $('sel_guidebook_lingshi').setStyle('display', '')
        } else {
          $('sel_guidebook_lingshi').setStyle('display', 'none')
        }
        if (this.value == 56973) {
          $('sel_xunlingzhu').setStyle('display', '')
          $('sel_xunlingzhu_level').setStyle('display', '')
        } else {
          $('sel_xunlingzhu').setStyle('display', 'none')
          $('sel_xunlingzhu_level').setStyle('display', 'none')
        }
        $$('#zhongling_stone_tx, #sel_zhongling_stone_level').setStyle(
          'display',
          this.value == 50344 ? '' : 'none'
        )
      })
      .fireEvent('change')
...
var LingshiKinds = null
function get_lingshi_kinds() {
  if (LingshiKinds == null) {
    LingshiKinds = get_subkinds_by_name(kind, '灵饰')
  }
  return LingshiKinds
}
function if_need_display_highlight() {
  return true
  var act = (window.CommonVar && CommonVar['act']) || ''
  var cur_storage_type = (window.CheckVar && CheckVar['cur_storage_type']) || ''
  if (
    [
      'search_role_equip',
      'search_role_equip_no_refer',
      'search_pet',
      'search_role',
      'search_lingshi'
    ].contains(act)
  ...
  if (act == 'query' && cur_storage_type == StorageStype['equip']) {
    ...
    var lingshi_kinds = get_lingshi_kinds()
    if (lingshi_kinds.contains(parseInt(QueryVar['kindid']))) {
      return true
    }
  }
  if (act == 'query' && cur_storage_type == StorageStype['pet']) {
    return true
  }
  if (act == 'query' && cur_storage_type == StorageStype['role']) {
    return true
  }
  if (act == 'overall_wjd_equip_search' || act == 'overall_wjd_lingshi_search') {
    return true
  }
  if (act == 'recommend_search') {
    var recommend_type = parseInt(QueryVar['recommend_type'])
    if (recommend_type == 2 || recommend_type == 3 || recommend_type == 1 || recommend_type == 4) {
      return true
    }
  }
  return false
...
function gen_highlight(highlights) {
  if (if_need_display_highlight()) {
    return '<td>' + gen_highlight_html(highlights, '<br />', false) + '</td>'
  } else {
    return ''
  }
}
function gen_dynamic_tags(dynamicTags) {
  return '<td>' + gen_dynamic_tags_html(dynamicTags, '<br />') + '</td>'
}
window.addEvent('domready', function () {
  if (if_need_display_highlight()) {
    $$('.jsGenHighlights').each(function ($el) {
      var highlights = JSON.decode($el.get('html'))
      $el.set({ html: gen_highlight_html(highlights, '<br />', false), style: { display: '' } })
    })
  }
})
...
function do_highlight_search() {
  if (CheckVar['cur_storage_type'] == StorageStype['pet']) {
    search_pet(1)
  } else if (CommonVar['act'] == 'search_lingshi') {
    search_lingshi(1)
...
function update_highlight_filter_panel() {
  ...
  render_to_replace('highlight_filter_panel', 'highlight_filter_templ', {
    filter_list: exists_filters
  })
}
function add_highlight_filter(el) {
  if (window.CommonVar && CommonVar.act && CommonVar.act.match(/search_(.*)/)) {
    var acts = CommonVar.act.match(/search_(.*)/)
    acts[1] === 'role_equip' && (acts[1] = 'equip')
    var elSelector = '#' + acts[1] + '_search'
    var $elSelector = $$(elSelector)
    $elSelector = $elSelector[0]
    if ($elSelector && !$elSelector.hasClass('on')) {
      $elSelector.click()
    }
  }
  var el = $(el)
