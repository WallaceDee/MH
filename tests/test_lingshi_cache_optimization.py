...
def test_cache_optimization():
    ...
    data1 = collector.get_market_data(kindid=61, limit=100)
    elapsed1 = time.time() - start_time
    print(f"第一次获取数据耗时: {elapsed1:.3f}秒，获取到 {len(data1)} 条数据")
    
    print("\n2. 第二次获取相同数据（应该使用实例缓存）")
    start_time = time.time()
    data2 = collector.get_market_data(kindid=61, limit=100)
    elapsed2 = time.time() - start_time
    print(f"第二次获取数据耗时: {elapsed2:.3f}秒，获取到 {len(data2)} 条数据")
    
    print("\n3. 第三次获取不同kindid数据（应该使用实例缓存过滤）")
    start_time = time.time()
    data3 = collector.get_market_data(kindid=62, limit=100)
    elapsed3 = time.time() - start_time
    print(f"第三次获取数据耗时: {elapsed3:.3f}秒，获取到 {len(data3)} 条数据")
    
    print("\n4. 第四次获取所有灵饰数据（应该使用实例缓存）")
    start_time = time.time()
    data4 = collector.get_market_data(limit=200)
    ...
    start_time = time.time()
    data5 = collector.get_market_data(kindid=61, limit=100)
    ...
    print("\n8. 缓存状态检查")
    if collector._cached_lingshi_data is not None:
        print(f"✅ 实例缓存有效，包含 {len(collector._cached_lingshi_data)} 条灵饰数据")
