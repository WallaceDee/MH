...
def test_service_layer():
    ...
    try:
        ...
        with app.app_context():
            service = RoleServiceORM()
            print(f"✅ 角色服务创建成功")
            
            # 测试获取角色列表
            result = service.get_roles_list(page=1, page_size=10)
            print(f"✅ 获取角色列表成功: {result['code']}")
            
            # 测试获取角色详情（如果存在）
            if result['data']['total'] > 0:
                first_role = result['data']['items'][0]
                detail = service.get_role_detail(first_role['eid'])
                if detail:
                    print(f"✅ 获取角色详情成功: {detail['equip_name']}")
                else:
                    print(f"⚠️  角色详情为空")
            else:
                print(f"⚠️  没有角色数据，跳过详情测试")
