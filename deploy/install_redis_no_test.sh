#!/bin/bash
# Redis快速安装脚本（跳过测试）- Alibaba Cloud Linux 3
# CBG爬虫项目Redis缓存系统安装指南
# 适用于: Alibaba Cloud Linux 3 (基于CentOS 8)
# 注意: 此版本跳过make test，适用于测试失败但需要快速安装的场景
...
install_redis() {
    ...
    log_info "开始安装Redis ${REDIS_VERSION}（跳过测试模式）..."
    ...
    # 编译Redis（跳过测试）
    log_warning "跳过make test以避免测试失败影响安装"
    make
    make install PREFIX=${REDIS_DIR}
    
    log_success "Redis编译安装完成（已跳过测试）"
...
start_and_test_redis() {
    ...
    # 运行基础功能测试
    log_info "运行基础功能测试..."
    redis-cli set test_key "CBG_Redis_Test" > /dev/null
    if [[ "$(redis-cli get test_key)" == "CBG_Redis_Test" ]]; then
        log_success "Redis读写功能正常"
        redis-cli del test_key > /dev/null
    else
        log_error "Redis读写功能异常"
        exit 1
    fi
}
...
show_installation_info() {
    ...
    log_info "测试失败说明: 内存碎片整理测试边界值问题，不影响实际使用"
}

# 主函数
main() {
    log_info "开始安装Redis for CBG项目（跳过测试模式）..."
