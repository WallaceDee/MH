window.addEvent('domready', function() {
  ...
  function get_now_time_str() {
      ...
      return fixZero(now.getHours()) + ':' + fixZero(now.getMinutes()) + ':' + fixZero(now.getSeconds());
  }
  var spriteDialog = ({
      $root: $('spriteQA'),
      cookie: 'sprite_open',
      init: function(opts) {
          ...
          ctx.$spriteContent = ctx.$sprite.getElement('.content');
      ...
      bindUI: function() {
          var ctx = this;
          ctx.$root.getElements('.qa-close').addEvent('click', function() {
              ctx.hide();
              return false;
          });
          ctx.$root.getElements('.tab').each(function($tab, index) {
          ...
          ctx.$root.addEvent('switchTab', function(index) {
              setTimeout(function() {
                  if (index == 1 && ctx.opts.initAnswer && !ctx.spriteContentHtml) {
          ...
          ctx.$root.addEvent('click:relay(.hot-content a)', quikAsk);
          ctx.$root.addEvent('click:relay(.qa-sprite [name=ask])', quikAsk);
          ctx.$root.addEvent('click:relay(.qa-sprite a)', function(e) {
              if (this.get('name') == 'ask') {
                  return;
              }
              var href = this.get('href');
              var mathcersServerid = href.match(/[&?]serverid=([^&]+)/);
              var matchersOrdersn = href.match(/[&?]ordersn=([^&]+)/);
              if (mathcersServerid && matchersOrdersn) {
                  var matchersViewloc = href.match(/[&?]view_loc=([^&]+)/);
                  var matchersEquipRefer = href.match(/[&?]equip_refer=([^&]+)/);
                  var matchersWebLog = href.match(/[&?]_web_log=([^&]+)/);
      ...
      switchTab: function(index) {
          ...
          var $contents = ctx.$root.getElements('.js-content');
          if ($contents[index]) {
              $contents.setStyle('display', 'none')[index].setStyle('display', 'block');
          }
      ...
      updateHotModule: function(options) {
          ...
          var $root = this.$root;
          if (opts.question) {
              $root.getElements('.hot-questions').set('html', window.render('sprite-hot-question-template', opts.question));
          }
      },
      ...
      addToSprintContent: function(isQuestion, opts) {
          ...
          var title = '<dt class="iconWrap"><img class="icon" src="' + opts.url + '" /><i class="zz"></i></dt>';
          var result = '<dl class="' + opts.className + '">\
      ' + (isQuestion ? '' : title) + '\
      <dd>\
        <div class="time">' + opts.time + '</div>\
        <div class="message">\
          <i class="tl"></i><i class="tr"></i><i class="bl"></i><i class="br"></i>\
          <div class="text">' + opts.html + '</div>\
        </div>\
      </dd>\
      ' + (isQuestion ? title : '') + '\
    </dl>';
          ctx.spriteContentHtml += result;
          ctx.$spriteContent.set('html', ctx.spriteContentHtml);
          ctx.fixScroll();
      },
      _scrollTimer: null,
      fixScroll: function() {
          ...
          ctx._scrollTimer = setTimeout(function() {
              ...
              var $last = ctx.$spriteContent.getLast('.qa-q') || ctx.$spriteContent.getLast();
              $main.scrollTop = $last.getOffsets().y - $main.getOffsets().y - 5;
          });
      },
      show: function() {
      ...
      bindEvaluateUI: function() {
          var ctx = this;
          var $evaluate = ctx.$evaluate = ctx.$root.getElement('.spritemm');
          $evaluate.addEvent('click:relay(.js_resolve)', function(e) {
              ctx.showGoodEvaluate();
              e.stop();
          }).addEvent('click:relay(.js_reject)', function(e) {
              ctx.showBadEvaluate();
              e.stop();
          });
      },
      ...
      showGoodEvaluate: function() {
          ...
          css.display = 'none';
          $evaluate.getElements('.js_init,.js_bad').setStyles(css);
      ...
      showBadEvaluate: function() {
          ...
          css.display = 'none';
          $evaluate.getElements('.js_init,.js_good').setStyles(css);
      ...
      showEvaluate: function() {
          ...
          css.display = 'none';
          $evaluate.getElements('.js_good,.js_bad').setStyles(css);
      },
  ...
  if (window.sprite_hotq_web) {
      var data = {
          updateTime: '',
          banner: {},
          list: []
      };
      var spriteList = sprite_hotq_web.split('||');
      data.updateTime = (spriteList.shift() || '').trim();
      if (!/^\d+-\d+-\d+/.test(data.updateTime)) {
          data.updateTime = '';
      }
      var images = (spriteList.shift() || '').split('@');
      var isImage = images[0] && /^(https?:)?\/\//.test(images[0].trim());
      var isImageLink = images[1] && /^(https?:)?\/\//.test(images[1].trim());
      data.banner = {
          src: isImage ? images[0] : '',
          text: htmlEncode(isImageLink ? '' : (images[1] || '')),
      ...
      if (!$spriteBtn.hasClass('on')) {
          var isUpdated = !Store.read(cookie_update_time) || data.updateTime > Store.read(cookie_update_time);
          if (isUpdated) {
              var $btn = $('sprite_link');
  ...
  function initAutoComplete() {
      ...
      ask_tips = new AutoComplete($('type_box'),{
          startPoint: 1,
          promptNum: 20,
          selectAtEnter: false,
          handle_func: function(keyword) {
              var __this = this;
              var url = '//tip.chatbot.nie.163.com/cgi-bin/good_evaluate_question_tip.py'
              new Request.JSONP({
                  url: url,
                  ...
                  onComplete: function(result) {
                      if (result.success) {
                          var data = result.data.map(function(v) {
                              return '·' + v
                          });
  ...
  function add_chat_question(question, is_question, original_size) {
      if (is_question) {
          spriteDialog.addQuestion(htmlEncode(question));
      } else {
  ...
  function ask(question, eval, nolog) {
      ...
      if (ques == '' || ques == $type_box.defaultValue) {
          ...
          if (user == '') {
              ...
              var fingerprint = Cookie.read('fingerprint') || '';
              server_name = decodeURIComponent(server_name);
              if (!Cookie.read('is_user_login')) {
                  urs = "";
                  roleid = "";
                  level = "";
              }
              user += 'channel=pc,urs=' + urs + ',hostnum=' + server_name + ',uid=' + roleid + ',grade=' + level + ',fingerprint=' + fingerprint;
          }
          var ask_url = location.origin + '/cgi-bin/sprite.py';
          var params = {
              act: 'knowledge',
              question: ques
          };
