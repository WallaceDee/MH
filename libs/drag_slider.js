var ButtonChecker = new Class({
    ...
    add_disable: function() {
        if (this.panel.getElements('li').length > 0) {
            this.reset_value();
            this.panel.getElements('li').addClass('disable');
        }
    },
    remove_disable: function() {
        if (this.panel.getElements('li').length > 0) {
            this.panel.getElements('li').removeClass('disable');
        }
    },
    ...
    remove_check_el: function(el) {
        el.removeClass('on');
        this.check_queue = this.check_queue.filter(function(x) {
            return x != el
        });
    },
    ...
    get_value: function() {
        return this.check_queue.map(function(x) {
            return x.retrieve('value')
        }).join(',');
    },
...
var PetSkillButtonChecker = new Class({
    ...
    create_element: function(item) {
        var icon = (item[2] ? item[2] : this.fill_format(item[0])) + '.gif';
        var icon_url = ResUrl + '/images/skill/' + icon;
        return new Element('li',{
            'html': '<img src="' + icon_url + '" alt="' + item[1] + '" />',
            'data-skill_id': item[0],
            'title': item[1]
        });
    },
...
var Drag = new Class({
    ...
    reg_event: function() {
        var __this = this;
        var isMobile = /iphone|ipod|ipad|ipad|Android|nokia|blackberry|webos|webos|webmate|bada|lg|ucweb/i.test(navigator.userAgent.toLowerCase());
        var eventConf = {
            down: 'mousedown',
            up: 'mouseup',
            move: 'mousemove'
        ...
        this.el.addEvent(eventConf.down, function(ev) {
            __this.start(ev);
            if (!isMobile) {
                ev.preventDefault();
            }
        });
        document.addEvent(eventConf.up, function(ev) {
            __this.end(ev);
        });
        document.addEvent(eventConf.move, function(ev) {
            __this.move(ev);
            if (!isMobile) {
                ev.preventDefault();
            }
    ...
    init_drag: function() {
        ...
        this.startDrag.set_options({
            'onMove': function(el, value, drag_value) {
                ...
                el.getElement('.num').set('html', level + options.tipSuffix);
                ...
                __this.endDrag.set_options({
                    limit: [value.x, __this.drag_limit[1]]
                });
