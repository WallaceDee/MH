...
class Pet(Base):
    ...
    equip_status_desc = Column(Text, comment='召唤兽状态描述')
    ...
    create_time_equip = Column(Text, comment='召唤兽创建时间')
    ...
    has_collect = Column(Integer, default=0, comment='是否收藏')
    ...
    equip_face_img = Column(Text, comment='召唤兽头像图片')
    ...
    is_split_main_role = Column(Integer, default=0, comment='是否拆分主角')
    is_split_independent_role = Column(Integer, default=0, comment='是否独立角色拆分')
    is_split_independent_equip = Column(Integer, default=0, comment='是否独立召唤兽拆分')
    split_equip_sold_happen = Column(Integer, default=0, comment='拆分召唤兽销售发生')
    show_split_equip_sold_remind = Column(Integer, default=0, comment='显示拆分召唤兽销售提醒')
    ...
    # 测试服相关
    equip_in_test_server = Column(Integer, default=0, comment='召唤兽在测试服')
    buyer_in_test_server = Column(Integer, default=0, comment='买家在测试服')
    equip_in_allow_take_away_server = Column(Integer, default=0, comment='召唤兽在允许带走服务器')
    
    # 其他标识
    is_weijianding = Column(Integer, default=0, comment='是否未鉴定')
    is_show_alipay_privilege = Column(Integer, default=0, comment='是否显示支付宝特权')
    is_seller_redpacket_flag = Column(Integer, default=0, comment='是否卖家红包标志')
    is_show_expert_desc = Column(Integer, default=0, comment='是否显示专家描述')
    is_show_special_highlight = Column(Integer, default=0, comment='是否显示特殊高亮')
    is_xyq_game_role_kunpeng_reach_limit = Column(Integer, default=0, comment='是否达到鲲鹏限制')
    ...
    # decode_desc解析出的字段
    pet_name = Column(Text, comment='解析出的召唤兽名称')
    type_id = Column(Text, comment='解析出的类型ID')
    pet_grade = Column(Text, comment='解析出的召唤兽等级')
    blood = Column(Text, comment='解析出的气血')
    magic = Column(Text, comment='解析出的魔法')
    attack = Column(Integer, default=0, comment='解析出的攻击')
    defence = Column(Text, comment='解析出的防御')
    speed = Column(Text, comment='解析出的速度')
    soma = Column(Text, comment='解析出的体质')
    magic_powner = Column(Text, comment='解析出的魔力')
    strength = Column(Text, comment='解析出的力量')
    endurance = Column(Text, comment='解析出的耐力')
    smartness = Column(Text, comment='解析出的敏捷')
    potential = Column(Text, comment='解析出的潜力')
    wakan = Column(Text, comment='解析出的灵力')
    max_blood = Column(Text, comment='解析出的最大气血')
    max_magic = Column(Text, comment='解析出的最大魔法')
    lifetime = Column(Text, comment='解析出的寿命')
    five_aptitude = Column(Text, comment='解析出的五行')
    attack_aptitude = Column(Text, comment='解析出的攻击资质')
    defence_aptitude = Column(Text, comment='解析出的防御资质')
    physical_aptitude = Column(Text, comment='解析出的体力资质')
    magic_aptitude = Column(Text, comment='解析出的法力资质')
    speed_aptitude = Column(Text, comment='解析出的速度资质')
    avoid_aptitude = Column(Text, comment='解析出的躲避资质')
    ...
    is_baobao = Column(Text, comment='解析出的是否宝宝')
    used_qianjinlu = Column(Text, comment='解析出的千金露使用情况')
    sp_skill_id = Column(Text, comment='解析出的特殊技能ID')
    all_skills = Column(Text, comment='解析出的技能列表')
    ...
    jinjie = Column(Text, comment='解析出的进阶信息')
    lx = Column(Integer, default=0, comment='解析出的灵性')
    jinjie_cnt = Column(Text, comment='解析出的进阶次数')
    texing = Column(Text, comment='解析出的特性')
    core_close = Column(Text, comment='解析出的核心关闭状态')
    ...
    neidan = Column(Text, comment='解析出的内丹')
    equip_list = Column(Text, comment='解析出的装备列表')
    evol_skill_list = Column(Text, comment='解析出的进化技能列表')
    evol_skills = Column(Text, comment='解析出的进化技能')
    ...
    other = Column(Text, comment='解析出的其他信息')
