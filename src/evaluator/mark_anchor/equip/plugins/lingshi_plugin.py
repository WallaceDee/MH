...
class LingshiPlugin(EquipmentTypePlugin):
    ...
    def get_derived_features(self, features: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 计算主属性标准化得分
        main_attr_scores = self._calculate_main_attr_scores(features, equip_level, config_data)
    ...
    def _load_lingshi_config(self) -> Dict[str, Any]:
        ...
        try:
            import json
            config_path = os.path.join(os.path.dirname(__file__), 'lingshi.jsonc')
        ...
        for attr_field, attr_name in main_attrs.items():
            ...
            if attr_value > 0:
                ...
                if len(attr_range) == 2:
                    ...
                    if max_val > min_val:
                        # 计算标准化得分 (0-1)
                        normalized_score = (attr_value - min_val) / (max_val - min_val)
                        # 限制在0-1范围内
                        ...
                        print(f"[主属性得分] {attr_name}: {attr_value} (范围: {min_val}-{max_val}) -> {score_100:.2f}分")
    ...
    def get_weight_overrides(self, kindid: int = None) -> Dict[str, float]:
        ...
        if kindid is not None:
            if kindid == 61:  # 戒指 - 只有伤害和防御
                base_weights.update({
                    'damage_score': 4.0,      # 戒指伤害得分
                    'defense_score': 4.0,     # 戒指防御得分
                })
            elif kindid == 62:  # 耳饰 - 只有法术伤害和法术防御
                base_weights.update({
          
                    'magic_damage_score': 4.0,    # 耳饰法术伤害得分
                    'magic_defense_score': 4.0,   # 耳饰法术防御得分
    
                })
            elif kindid == 63:  # 手镯 - 只有封印命中等级和抵抗封印等级
                base_weights.update({
                    'fengyin_score': 4.0,         # 手镯封印命中得分
                    'anti_fengyin_score': 4.0,    # 手镯抵抗封印得分
                })
            elif kindid == 64:  # 佩饰 - 只有速度
                base_weights.update({
                    'speed_score': 4.0,           # 佩饰速度得分
                })
