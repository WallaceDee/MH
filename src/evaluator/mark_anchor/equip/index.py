...
class EquipmentPluginManager:
    ...
    def register_plugin(self, plugin: EquipmentTypePlugin):
        ...
        for kindid in plugin.supported_kindids:
            ...
            # 按优先级排序
            self._kindid_plugin_map[kindid].sort(key=lambda p: p.priority, reverse=True)
    ...
    def get_final_tolerances(self, kindid: int) -> Dict[str, float]:
        ...
        for plugin in self.get_plugins_for_kindid(kindid):
            ...
            for key, increment in rel_increments.items():
                relative_tolerances[key] = relative_tolerances.get(key, 0) + increment
...
class EquipAnchorEvaluator:
    ...
    def find_market_anchors(self,
        ...
        try:
            ...
            needs_addon_filter = self.base_config.needs_addon_classification_filter(target_kindid)
            ...
            if self.base_config.is_lingshi(target_kindid):
                ...
                market_data = self.lingshi_market_collector.get_market_data(
                    ...
                    main_attr=pre_filters.get('main_attr'),
                    attrs=pre_filters.get('attrs'),
                    is_super_simple=pre_filters.get('is_super_simple'),
                    limit=1000
                )
    ...
    def _build_pre_filters(self, target_features: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.base_config.is_lingshi(kindid):
            ...
            # 灵饰装备主属性候选
            lingshi_main_attrs = ['damage', 'defense', 'magic_damage', 'magic_defense', 'fengyin', 'anti_fengyin', 'speed']
            
            for attr_name in lingshi_main_attrs:
                attr_value = target_features.get(attr_name, 0)
                if attr_value > 0:
                    filters['main_attr'] = attr_name
                    print(f"[灵饰主属性] kindid={kindid}, 主属性={attr_name}, 值={attr_value}")
                    break  # 只取第一个找到的主属性
    ...
    def _calculate_similarity(self,
        ...
        try:
            ...
            # 获取装备类型相关配置（通过插件系统）
            kindid = target_features.get('kindid', 0)
            feature_weights, relative_tolerances = self.get_equip_type_configs(kindid)
            
            # 特征增强：添加派生特征
            enhanced_target_features = self.plugin_manager.get_enhanced_features(kindid, target_features)
            enhanced_market_features = self.plugin_manager.get_enhanced_features(kindid, market_features)

            total_weight = 0
            weighted_similarity = 0

            # 合并所有特征名称（包括派生特征）
            all_features = set(enhanced_target_features.keys()) | set(enhanced_market_features.keys())
            # 过滤掉一些元数据字段
            excluded_features = {'equip_sn', 'price', 'server_id', 'create_time'}
            ...
            for feature_name in all_features:
                ...
                if plugin_similarity is not None:
                    ...
                    feature_similarity = self._calculate_suit_effect_similarity(target_val, market_val, target_kindid, market_kindid)
                    ...
                    if feature_name == 'repair_fail_num':
                        ...
                        if target_val == market_val:
                            ...
                            feature_similarity = max(0.0, 1.0 - diff * 0.3)  # 每多1次失败，相似度降低0.3
                        else:
                            # 目标装备修理失败次数更多，相似度降低较少
                            diff = target_val - market_val
                            feature_similarity = max(0.0, 1.0 - diff * 0.2)  # 每多1次失败，相似度降低0.2
                        ...
                        try:
                            ...
                            if isinstance(target_val, (list, dict)) or isinstance(market_val, (list, dict)):
                                ...
                                print(f"  目标值: {target_val} (类型: {type(target_val)})")
                                print(f"  市场值: {market_val} (类型: {type(market_val)})")
                                ...
                                # 确保都是数值类型后进行计算
                                target_numeric = float(target_val) if target_val is not None else 0
                                market_numeric = float(market_val) if market_val is not None else 0
                                
                                denominator = max(abs(target_numeric), abs(market_numeric))
                                diff_ratio = abs(target_numeric - market_numeric) / denominator
            ...
            print("-" * 90)
            print(f"{'总计':20s} | {'':8s} | {'':8s} | {total_weight:7.2f} | {'':7s} | {weighted_similarity:7.3f} | {'':6s}")
            print(f"最终相似度: {weighted_similarity:.3f} / {total_weight:.3f} = {weighted_similarity/total_weight if total_weight > 0 else 0:.3f}")
            print("=" * 90)
    ...
    def calculate_value(self,
        ...
        """
        try:
            print(f"开始计算装备价值，策略: {strategy}，相似度阈值: {similarity_threshold}，最大锚点数: {max_anchors}")
