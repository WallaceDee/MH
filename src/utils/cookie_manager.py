...
class CookieManager:
    ...
    def _get_cookie_path(self) -> str:
        ...
        self.logger.debug(f"Cookie文件路径: {cookie_path}")
        return cookie_path
...
def update_cookies_with_playwright(logger: Optional[logging.Logger] = None):
    ...
    try:
        ...
        try:
            ...
            # 如果在异步环境中，直接返回False（避免嵌套事件循环）
            log.warning("在异步环境中无法更新Cookie，请使用异步版本的更新函数")
            return False
        except RuntimeError:
            # 不在异步环境中，可以安全调用asyncio.run
            pass
...
async def verify_cookie_validity_async(logger: Optional[logging.Logger] = None):
    """
    异步验证Cookie的有效性
    ...
    log.info("=== 开始异步验证Cookie有效性 ===")
    
    try:
        log.info("准备导入playwright.async_api...")
        from playwright.async_api import async_playwright
        log.info("playwright.async_api导入成功")
    except Exception as e:
        log.error(f"导入playwright.async_api失败: {e}", exc_info=True)
        return False
    
    async def check_with_playwright(cookie_str):
        """使用Playwright检查Cookie有效性的异步函数"""
        log.info("=== 进入check_with_playwright异步函数 ===")
        try:
            log.info("启动Playwright浏览器...")
            async with async_playwright() as p:
                log.info("Playwright上下文创建成功")
                # 启动无头浏览器
                browser = await p.chromium.launch(headless=True)
                log.info("Chromium浏览器启动成功")
                context = await browser.new_context()
                log.info("浏览器上下文创建成功")
                
                # 设置Cookie
                cookies = []
                log.info(f"开始解析Cookie字符串，长度: {len(cookie_str)}")
                for cookie in cookie_str.split('; '):
                    if '=' in cookie:
                        ...
                        cookies.append({
                            'name': name,
                            'value': value,
                            'domain': '.163.com',  # 使用正确的域名
                            'path': '/'
                        })
                
                log.info(f"解析完成，设置Cookie数量: {len(cookies)}")
                if cookies:
                    log.info(f"第一个Cookie: {cookies[0]['name']}={cookies[0]['value'][:10]}...")
                
                await context.add_cookies(cookies)
                log.info("Cookie设置到浏览器上下文成功")
                ...
                log.info(f"开始访问测试URL: {test_url}")
                await page.goto(test_url, wait_until='networkidle', timeout=30000)
                log.info("页面加载完成")
                
                # 检查最终URL是否重定向到登录页
                final_url = page.url
                log.info(f"最终URL: {final_url}")
                is_login_page = 'show_login.py' in final_url or 'login' in final_url.lower()
                
                await browser.close()
                log.info("浏览器已关闭")
                
                # 简化判断：只要没有重定向到登录页就认为Cookie有效
                is_valid = not is_login_page
                
                log.info(f"Cookie验证详情: 原始URL={test_url}, 最终URL={final_url}, 是否登录页={is_login_page}, Cookie有效={is_valid}")
                
                return is_valid
                
        except Exception as e:
            log.error(f"使用Playwright验证Cookie时出错: {e}", exc_info=True)
            return False

    try:
        log.info("开始获取Cookie管理器实例...")
        # 获取Cookie管理器实例
        cookie_manager = get_cookie_manager(logger)
        log.info("Cookie管理器实例获取成功")
        
        log.info("开始获取Cookie内容...")
        cookie_content = cookie_manager.get_cookie_content()
        log.info(f"Cookie内容获取结果: {cookie_content!r}")
        
        if not cookie_content:
            log.warning("Cookie内容为空，直接返回False")
            return False
        
        log.info(f"获取到Cookie内容，长度: {len(cookie_content)}")
        
        # 直接调用异步验证函数
        log.info("开始异步验证Cookie...")
        result = await check_with_playwright(cookie_content)
        log.info(f"异步验证结果: {result}")
        return result
            
    except Exception as e:
        log.error(f"验证Cookie有效性时出错: {e}", exc_info=True)
        return False


def verify_cookie_validity(logger: Optional[logging.Logger] = None):
    """
    ...
    try:
        ...
        # 在同步环境中调用异步函数
        log.info("在同步环境中验证Cookie...")
        result = asyncio.run(verify_cookie_validity_async(logger))
        log.info(f"同步验证结果: {result}")
        return result
